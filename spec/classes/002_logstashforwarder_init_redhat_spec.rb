      ...
      context 'package installation' do
        ...
        context 'via repository' do
          ...
          context 'when setting package version and package_url' do
            ...
            let (:params) {
              default_params.merge({
                :version     => '0.90.10',
                :package_url => 'puppet:///path/to/some/logstashforwarder-0.90.10.rpm'
              })
            }
        ...
        context 'via package_url setting' do
          ...
          context 'using puppet:/// schema' do
            ...
            it { should contain_file('/opt/logstashforwarder/swdl/package.rpm').with(:source => 'puppet:///path/to/package.rpm', :backup => false) }
            it { should contain_package('logstash-forwarder').with(:ensure => 'present', :source => '/opt/logstashforwarder/swdl/package.rpm', :provider => 'rpm') }
          end
          ...
          context 'using http:// schema' do
            ...
            it { should contain_exec('create_package_dir_logstashforwarder').with(:command => 'mkdir -p /opt/logstashforwarder/swdl') }
            it { should contain_file('/opt/logstashforwarder/swdl').with(:purge => false, :force => false, :require => "Exec[create_package_dir_logstashforwarder]") }
            it { should contain_exec('download_package_logstashforwarder').with(:command => 'wget -O /opt/logstashforwarder/swdl/package.rpm http://www.domain.com/path/to/package.rpm 2> /dev/null', :require => 'File[/opt/logstashforwarder/swdl]') }
            it { should contain_package('logstash-forwarder').with(:ensure => 'present', :source => '/opt/logstashforwarder/swdl/package.rpm', :provider => 'rpm') }
          end
          ...
          context 'using https:// schema' do
            ...
            it { should contain_exec('create_package_dir_logstashforwarder').with(:command => 'mkdir -p /opt/logstashforwarder/swdl') }
            it { should contain_file('/opt/logstashforwarder/swdl').with(:purge => false, :force => false, :require => 'Exec[create_package_dir_logstashforwarder]') }
            it { should contain_exec('download_package_logstashforwarder').with(:command => 'wget -O /opt/logstashforwarder/swdl/package.rpm https://www.domain.com/path/to/package.rpm 2> /dev/null', :require => 'File[/opt/logstashforwarder/swdl]') }
            it { should contain_package('logstash-forwarder').with(:ensure => 'present', :source => '/opt/logstashforwarder/swdl/package.rpm', :provider => 'rpm') }
          end
          ...
          context 'using ftp:// schema' do
            ...
            it { should contain_exec('create_package_dir_logstashforwarder').with(:command => 'mkdir -p /opt/logstashforwarder/swdl') }
            it { should contain_file('/opt/logstashforwarder/swdl').with(:purge => false, :force => false, :require => 'Exec[create_package_dir_logstashforwarder]') }
            it { should contain_exec('download_package_logstashforwarder').with(:command => 'wget -O /opt/logstashforwarder/swdl/package.rpm ftp://www.domain.com/path/to/package.rpm 2> /dev/null', :require => 'File[/opt/logstashforwarder/swdl]') }
            it { should contain_package('logstash-forwarder').with(:ensure => 'present', :source => '/opt/logstashforwarder/swdl/package.rpm', :provider => 'rpm') }
          end
          ...
          context 'using file:// schema' do
            ...
            it { should contain_exec('create_package_dir_logstashforwarder').with(:command => 'mkdir -p /opt/logstashforwarder/swdl') }
            it { should contain_file('/opt/logstashforwarder/swdl').with(:purge => false, :force => false, :require => 'Exec[create_package_dir_logstashforwarder]') }
            it { should contain_file('/opt/logstashforwarder/swdl/package.rpm').with(:source => '/path/to/package.rpm', :backup => false) }
            it { should contain_package('logstash-forwarder').with(:ensure => 'present', :source => '/opt/logstashforwarder/swdl/package.rpm', :provider => 'rpm') }
          end
      ...
      context 'service setup' do
        ...
        context 'with provider \'init\'' do
          ...
          context 'and set defaults via hash param' do
            ...
            it { should contain_file('/etc/sysconfig/logstash-forwarder').with(:content => "### MANAGED BY PUPPET ###\n\nSERVICE_GROUP=root\nSERVICE_USER=root\n") }
          ...
          context 'and set defaults via file param' do
            ...
            it { should contain_file('/etc/sysconfig/logstash-forwarder').with(:source => 'puppet:///path/to/logstashforwarder.defaults') }
